...
#separate the simplices and appears_at to a different function
...
#Distance matrix: designed for Rips. but also accepts simplices
class simplicial_pyvis:
    def __init__(self, max_dim, distance_mat=None, simplices=None, cycles=None, cycle_dim=None, index_to_name=None, only_neiborhood=False):
        if distance_mat is None and simplices is None:
            raise ValueError("Provide either distance_mat or simplices.")
        if distance_mat is not None and simplices is not None:
            raise ValueError("Provide only one of distance_mat or simplices, not both.")

        self.D = distance_mat
        #dictionary: "0": 0-simplcies, "1": 1-simplices
        self.simplices = simplices
        self.max_dim = max_dim
        # --- cycle-related ---
        #in the form of list of list of simplices, e.g: [[[1,4],[3,4]],...]
        self.cycles = cycles
   ...
   #this needs to be added first, because the package does not overwrite
    def add_cycles(self):
        ...
        if cycle_dim == 1:
            for cycle in self.cycles:
                for edge in cycle:
                    source, target = edge
                    self.net.add_edge(source, target, value=G[source][target]["weight"], color="red") 
        else:
    ...
    def add_graph_to_net(self):
        if self.D is not None:
            ...
            for node in nodes:
                self.net.add_node(node,label=index_to_name[node])
            for ind in edge_pos:
                self.net.add_edge(ind[0],ind[1],width=1/(max(D[ind[0],ind[1]],1e-5)))
        elif self.simplices is not None:
            nodes = simplices["0"]
            edges = simplices["1"]
