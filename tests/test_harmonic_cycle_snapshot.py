...
from network_lab_tda.tda_analysis.harmonic_cycle_snapshot import harmonic_cycle_snapshot
...
def _matilda_harmonic_at_threshold(D, cycle_dim, threshold, log_path):
    ...
    # isn't comparable to harmonic_cycle_snapshot, which always projects onto the
...
def test_square_single_cycle_matches_persistent_harmonic(square_D, tmp_path):
    ...
    snap = harmonic_cycle_snapshot(square_D, thresholds=[threshold], log_path=str(tmp_path / "snap.json"))
    snapshot_cycles = snap.run_snapshot(save=False)
...
def test_line_produces_no_cycles_in_both(line_D, tmp_path):
    ...
    snap = harmonic_cycle_snapshot(line_D, thresholds=[threshold], log_path=str(tmp_path / "snap.json"))
    snapshot_cycles = snap.run_snapshot(save=False)
...
def test_two_disjoint_squares_harmonic_subspace_matches(two_squares_D, tmp_path):
    ...
    snap = harmonic_cycle_snapshot(two_squares_D, thresholds=[threshold], log_path=str(tmp_path / "snap.json"))
    snapshot_cycles = snap.run_snapshot(save=False)
...
def test_two_squares_sharing_edge_harmonic_subspace_matches(shared_edge_squares_D, tmp_path):
    ...
    snap = harmonic_cycle_snapshot(shared_edge_squares_D, thresholds=[threshold], log_path=str(tmp_path / "snap.json"))
    snapshot_cycles = snap.run_snapshot(save=False)
...
def test_point_cloud_matches_matilda_harmonic_at_same_threshold(point_cloud_D, tmp_path):
    ...
    snap = harmonic_cycle_snapshot(point_cloud_D, thresholds=[threshold], log_path=str(tmp_path / "snap.json"))
    snapshot_cycles = snap.run_snapshot(save=False)
...
def test_matches_harmonic_cycle_snapshots_at_same_threshold(point_cloud_D, tmp_path):
    # harmonic_cycle_snapshot builds the harmonic basis directly, via a stacked
    # null_space([boundary_p; boundary_p1.T]) -- a different route than
    # harmonic_cycle_snapshots' orth()-then-project-and-subtract. Both should still
    # land on the same harmonic subspace at a threshold with a nontrivial boundary space.
    threshold = 8.2

    multi = harmonic_cycle_snapshots(point_cloud_D, thresholds=[threshold], log_path=str(tmp_path / "multi.json"))
    multi_cycles = multi.run_snapshots(save=False)[threshold]

    single = harmonic_cycle_snapshot(point_cloud_D, thresholds=[threshold], log_path=str(tmp_path / "single.json"))
    single_cycles = single.run_snapshot(save=False)

    assert len(multi_cycles) == len(single_cycles) == 1

    edge_keys = sorted({tuple(e["simplex"]) for cycle in multi_cycles + single_cycles for e in cycle["edges"]})
    multi_matrix = np.column_stack([_edge_vector(c, edge_keys) for c in multi_cycles])
    single_matrix = np.column_stack([_edge_vector(c, edge_keys) for c in single_cycles])

    coeffs, *_ = np.linalg.lstsq(multi_matrix, single_matrix, rcond=None)
    residual = single_matrix - multi_matrix @ coeffs
    assert residual == pytest.approx(np.zeros_like(residual), abs=1e-6)


def test_stacked_null_space_method_matches_fundamental_cycle(point_cloud_D, tmp_path):
    threshold = 8.2

    fundamental = harmonic_cycle_snapshot(point_cloud_D, thresholds=[threshold], log_path=str(tmp_path / "f.json"))
    fundamental_cycles = fundamental.run_snapshot(save=False)

    stacked = harmonic_cycle_snapshot(
        point_cloud_D, thresholds=[threshold], log_path=str(tmp_path / "s.json"), method="stacked_null_space"
    )
    stacked_cycles = stacked.run_snapshot(save=False)

    assert len(fundamental_cycles) == len(stacked_cycles) == 1

    edge_keys = sorted({tuple(e["simplex"]) for c in fundamental_cycles + stacked_cycles for e in c["edges"]})
    fundamental_matrix = np.column_stack([_edge_vector(c, edge_keys) for c in fundamental_cycles])
    stacked_matrix = np.column_stack([_edge_vector(c, edge_keys) for c in stacked_cycles])

    coeffs, *_ = np.linalg.lstsq(fundamental_matrix, stacked_matrix, rcond=None)
    residual = stacked_matrix - fundamental_matrix @ coeffs
    assert residual == pytest.approx(np.zeros_like(residual), abs=1e-6)


def test_fundamental_cycle_method_rejects_higher_cycle_dim():
    with pytest.raises(ValueError):
        harmonic_cycle_snapshot(np.zeros((1, 1)), thresholds=[1.0], cycle_dim=2, method="fundamental_cycle")
